...
size_t USBD_Keyboard_press(uint8_t k)
{
    ...
    { // it's a printing key
        ...
        {                                 // it's a capital letter or other character reached with shift
            keyboardhid.MODIFIER |= 0x02; // the left shift modifier
            k &= 0x7F;
        }
    }
    ...
    if (keyboardhid.KEYCODE[0] != k && keyboardhid.KEYCODE[1] != k &&
        keyboardhid.KEYCODE[2] != k && keyboardhid.KEYCODE[3] != k &&
        keyboardhid.KEYCODE[4] != k && keyboardhid.KEYCODE[5] != k)
    {

        for (i = 0; i < 6; i++)
        {
            if (keyboardhid.KEYCODE[i] == 0x00)
            {
                keyboardhid.KEYCODE[i] = k;
                break;
            }
        }
        if (i == 6)
...
size_t USBD_Keyboard_release(uint8_t k)
{
    ...
    for (i = 0; i < 6; i++)
    {
        if (0 != k && keyboardhid.KEYCODE[i] == k)
        {
            keyboardhid.KEYCODE[i] = 0x00;
        }
    }
    USBD_HID_SendReport(&hUsbDeviceFS, (uint8_t*)&keyboardhid, sizeof(keyboardhid));
    return 1;
...
void USBD_Keyboard_releaseAll(void)
{
    ...
    keyboardhid.KEYCODE[1] = 0;
    keyboardhid.KEYCODE[2] = 0;
    keyboardhid.KEYCODE[3] = 0;
    keyboardhid.KEYCODE[4] = 0;
    keyboardhid.KEYCODE[5] = 0;
...
{
    USBD_Keyboard_press(key);
    while(USBD_Keyboard_State() != HID_IDLE)
        ;
    USBD_Keyboard_release(key);
    while(USBD_Keyboard_State() != HID_IDLE)
        ;
}